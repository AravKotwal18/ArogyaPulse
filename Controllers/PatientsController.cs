...
namespace ArogyaPulse.Api.Controllers
{
    ...
    public class PatientsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update(int id, [FromBody] PatientUpdateDto updateDto)
        {
            ...
            if (!string.IsNullOrWhiteSpace(updateDto.Gender)) existing.Gender = updateDto.Gender;
            if (!string.IsNullOrWhiteSpace(updateDto.BloodGroup)) existing.BloodGroup = updateDto.BloodGroup;
            if (!string.IsNullOrWhiteSpace(updateDto.Village)) existing.Village = updateDto.Village;
            if (existing.Gender != "Female") existing.IsPregnant = false;
            else if (updateDto.IsPregnant.HasValue) existing.IsPregnant = updateDto.IsPregnant.Value;
